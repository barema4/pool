...
import * as budgetTemplatesApi from '@/api/budgetTemplates'
...
// --- Saved budget templates (org's own + inherited from a managing agency,
// lazy-loaded on first Budget tab activation) ---
const budgetTemplates = ref<BudgetTemplate[]>([])
const budgetTemplatesLoading = ref(false)
const budgetTemplatesError = ref('')
const budgetTemplatesLoadedOnce = ref(false)

async function loadBudgetTemplates() {
  if (!store.event?.organizationId) return
  budgetTemplatesLoading.value = true
  budgetTemplatesError.value = ''
  try {
    budgetTemplates.value = await budgetTemplatesApi.listForOrganization(store.event.organizationId)
  } catch (err) {
    budgetTemplatesError.value = extractErrorMessage(err)
  } finally {
    budgetTemplatesLoading.value = false
  }
}

const selectedTemplateId = ref('')
const applyingSavedTemplate = ref(false)

async function handleApplySavedTemplate() {
  if (!selectedTemplateId.value) return
  templateError.value = ''
  applyingSavedTemplate.value = true
  try {
    await budgetTemplatesApi.apply(selectedTemplateId.value, eventId)
    ...
    applyingSavedTemplate.value = false
  }
}
...
async function handleSaveAsTemplate() {
  ...
  try {
    await budgetTemplatesApi.createFromEvent({
      organizationId: store.event.organizationId,
      eventId,
    ...
    saveTemplateError.value = extractErrorMessage(err)
  } finally {
...
async function handleDeleteTemplate(templateId: string) {
  ...
  budgetTemplatesError.value = ''
  try {
    await budgetTemplatesApi.remove(templateId)
    await loadBudgetTemplates()
  } catch (err) {
    budgetTemplatesError.value = extractErrorMessage(err)
  } finally {
...
function selectTab(tab: Tab) {
  ...
  if (tab === 'budget' && !budgetTemplatesLoadedOnce.value) {
    budgetTemplatesLoadedOnce.value = true
    loadBudgetTemplates()
  }
    ...
    <template v-else-if="store.event">
      ...
      <section v-else-if="activeTab === 'budget'">
        ...
        <template v-if="store.event.budgetingEnabled">
          ...
          <!-- Saved templates (org's own + inherited from a managing agency) -->
          <div
            v-if="catLoadedOnce && !catListLoading && catTotal === 0 && budgetTemplates.length > 0"
            class="mb-4 flex flex-wrap items-end gap-2 rounded-2xl border border-babyblue-100 bg-white p-4 shadow-sm"
          >
            <div class="min-w-48 flex-1">
              <label class="mb-1 block text-xs font-medium text-slate-700">📋 Apply a saved template</label>
              <select v-model="selectedTemplateId" :class="inputClass">
                <option value="">Select a template…</option>
                <option v-for="t in budgetTemplates" :key="t.id" :value="t.id">
            ...
            <button
              type="button"
              :disabled="!selectedTemplateId || applyingSavedTemplate"
              :class="outlineButtonClass"
              @click="handleApplySavedTemplate"
            >
              {{ applyingSavedTemplate ? 'Applying…' : 'Apply' }}
            </button>
          </div>

          <!-- Save current categories as a reusable template -->
          <div
            v-if="canManageBudget && catTotal > 0"
            class="mb-4 rounded-2xl border border-dashed border-babyblue-200 bg-babyblue-50/60 p-4"
          >
            <div class="flex flex-wrap items-center justify-between gap-3">
              <p class="text-sm text-slate-600">Reuse this budget's breakdown for future events.</p>
              <button
                type="button"
                :class="[outlineButtonClass, 'shrink-0']"
            ...
            <form
              ...
              class="mt-3 flex flex-wrap items-end gap-2 border-t border-babyblue-100 pt-3"
              @submit.prevent="handleSaveAsTemplate"
            >
              <div class="min-w-48 flex-1">
                <label class="mb-1 block text-xs font-medium text-slate-700">Template name</label>
                <input v-model="saveTemplateName" required :class="inputClass" />
              </div>
              <button type="submit" :disabled="savingTemplate" :class="primaryButtonClass">
                {{ savingTemplate ? 'Saving…' : 'Save' }}
              </button>
            </form>
            <p v-if="saveTemplateError" class="mt-2 text-sm text-red-600">{{ saveTemplateError }}</p>
          </div>

          <!-- Manage saved templates -->
          <div v-if="budgetTemplates.length > 0" class="mb-4">
            <p class="mb-2 text-xs font-medium text-slate-500">Saved templates</p>
            <ul class="space-y-1.5">
              <li
                v-for="t in budgetTemplates"
                :key="t.id"
              ...
              >
                <span class="min-w-0 truncate">{{ t.name }} <span class="text-xs text-slate-400">({{ t.items.length }} items)</span></span>
                <button
                  v-if="canManageBudget"
                  type="button"
                  :disabled="deletingTemplateId === t.id"
                  class="shrink-0 rounded-lg border border-red-200 px-2.5 py-1 text-xs font-medium text-red-600 transition-colors hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <p v-if="budgetTemplatesError" class="mb-3 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600">{{ budgetTemplatesError }}</p>
