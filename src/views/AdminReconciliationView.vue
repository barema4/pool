  ...
  <AdminLayout>
    ...
    <div v-else-if="error" class="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600">
      ...
      <p class="mt-1 text-xs text-red-500">
        This usually means the PawaPay API credentials aren't configured for live calls yet.
      </p>
    </div>

    <div v-else-if="report" class="grid gap-6 sm:grid-cols-2">
      <section
        v-for="country in report.pawapay"
        :key="country.countryCode"
        class="rounded-2xl border border-babyblue-100 bg-white p-5 shadow-sm"
      >
        <h2 class="mb-3 text-sm font-semibold tracking-wide text-babyblue-700 uppercase">
          {{ country.label }} (PawaPay)
        </h2>
        <dl class="space-y-2 text-sm">
          <div v-for="b in country.liveBalances" :key="b.currency" class="flex justify-between">
          ...
          <div v-if="country.liveBalances.length === 0" class="text-slate-400">No balance data returned.</div>
          <div class="flex justify-between">
            <dt class="text-slate-500">Owed to orgs</dt>
            <dd class="text-slate-900">{{ formatMoney(country.totalOwedToOrgs, country.currency) }}</dd>
          </div>
          <div class="flex justify-between">
            <dt class="text-slate-500">Platform fees</dt>
            <dd class="text-slate-900">{{ formatMoney(country.totalPlatformFees, country.currency) }}</dd>
          </div>
          <div class="flex justify-between border-t border-babyblue-100 pt-2 font-semibold">
            <dt class="text-slate-700">Expected total</dt>
            <dd class="text-slate-900">{{ formatMoney(country.expectedTotal, country.currency) }}</dd>
          </div>
          <div
            class="flex justify-between font-semibold"
            :class="Math.abs(country.drift) < 0.01 ? 'text-green-700' : 'text-red-600'"
          >
            <dt>Drift</dt>
            <dd>{{ formatMoney(country.drift, country.currency) }}</dd>
          </div>
        </dl>
      </section>
    </div>
