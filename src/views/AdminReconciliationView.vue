...
<template>
  <AdminLayout>
    ...
    <div v-else-if="report" class="grid gap-6 sm:grid-cols-2">
      ...
      <section class="rounded-2xl border border-babyblue-100 bg-white p-5 shadow-sm">
        <h2 class="mb-3 text-sm font-semibold tracking-wide text-babyblue-700 uppercase">🌍 Stripe (worldwide)</h2>
        <dl class="space-y-2 text-sm">
          <div v-for="b in report.stripe.liveBalances" :key="b.currency" class="flex justify-between">
            <dt class="text-slate-500">Live balance ({{ b.currency }})</dt>
            <dd class="font-semibold text-slate-900">{{ formatMoney(b.balance, b.currency) }}</dd>
          </div>
          <div v-if="report.stripe.liveBalances.length === 0" class="text-slate-400">No balance data returned.</div>
          <div class="flex justify-between border-t border-babyblue-100 pt-2">
            <dt class="text-slate-500">Expected platform fees</dt>
            <dd class="text-slate-900">{{ formatMoney(report.stripe.expectedPlatformFees) }}</dd>
          </div>
        </dl>
        <p class="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700">{{ report.stripe.caveat }}</p>
      </section>
    </div>
  </AdminLayout>
</template>
